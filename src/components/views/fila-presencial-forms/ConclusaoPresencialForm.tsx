...
export function ConclusaoPresencialForm() {
  const [data, setData, resetData] = useLocalStorage("presencial-conclusao-data", "");
  const [nome, setNome, resetNome] = useLocalStorage("presencial-conclusao-nome", "");
  const [pib, setPib, resetPib] = useLocalStorage("presencial-conclusao-pib", "");
  const [ip, setIp, resetIp] = useLocalStorage("presencial-conclusao-ip", "");
  const [pibMicro, setPibMicro, resetPibMicro] = useLocalStorage("presencial-conclusao-pib-micro", "");
  ...
  return (
    <div className="mt-3 space-y-2">
      ...
      <FormActions 
        ...
        onReset={handleReset} 
        copyMessage="Nota de Conclusão - Presencial copiada!" 
      />
    </div>
  );
}
